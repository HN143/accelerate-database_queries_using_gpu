...
select  i_item_id
 ...
 and d_date between cast('1999-05-28' as date) 
 				and (cast('1999-05-28' as date) + interval 30 days)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select  i_item_id
 ...
 and d_date between cast('2002-03-09' as date) and (cast('2002-03-09' as date) + interval 60 days)
...
select  
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('1999-06-27' as date) - interval 30 days)
                and (cast ('1999-06-27' as date) + interval 30 days) 
 group by
    w_state,i_item_id
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and store.s_city in ('Midway','Oak Grove','Fairview','Fairview','Fairview') 
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('cream','ghost','almond'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2002
 and     d_moy                   = 1
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('dodger','antique','snow','lavender','olive','wheat') and
...
select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1998
 	and t_time between 51577 and 51577+28800 
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('1998-03-21' as date) and (cast('1998-03-21' as date) + interval 60 days)
 and i_manufact_id in (576,251,691,286)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1999-02-16','1999-08-25','1999-11-22')))
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Brownsville'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  42756
   and ib_upper_bound   <=  42756 + 50000
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1999) and
        ((i_category in ('Jewelry','Home','Children') and
          i_class in ('mens watch','lighting','infants')
         )
      or (i_category in ('Shoes','Sports','Women') and
          i_class in ('athletic','sailing','dresses') 
        ))
