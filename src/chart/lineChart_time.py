#Biểu đồ đường so sánh thông số về Execution time của DuckDB và HeavyDB trên từng bộ dữ liệu (1G, 10G, 100G)
import pandas as pd
...
def plot_benchmark(size):
    ...
    duckdb_times = df_duckdb["Execution Time (ms)"].values
    heavydb_times = df_heavydb["Execution Time (ms)"].values
    ...
    plt.xticks(rotation=90)  #xoay nhãn 45 độ
    plt.show()
