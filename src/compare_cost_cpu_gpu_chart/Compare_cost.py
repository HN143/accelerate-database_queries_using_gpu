...
# Xử lý giá trị không hợp lệ trong cột "Giá Thuê AWS (VND/giờ)" và "Thời Gian Chạy DB Query (giây)"
all_data["Giá Thuê AWS (VND/giờ)"] = pd.to_numeric(all_data["Giá Thuê AWS (VND/giờ)"], errors="coerce").fillna(0)
...
# Dữ liệu cho cột (Giá thuê AWS)
cpu_rent_prices = all_data[all_data["Type"] == "CPU"]["Giá Thuê AWS (VND/giờ)"].values
gpu_rent_prices = all_data[all_data["Type"] == "GPU"]["Giá Thuê AWS (VND/giờ)"].values
...
# Vẽ cột (Giá thuê AWS)
ax1.bar(x_cpu, cpu_rent_prices, bar_width, color="blue", hatch="//", label="Giá Thuê AWS (CPU)")
ax1.bar(x_gpu, gpu_rent_prices, bar_width, color="green", label="Giá Thuê AWS (GPU)")
...
# Cài đặt trục OY
ax1.set_ylabel("Giá Thuê AWS (VND/giờ)", color="blue")
ax2.set_ylabel("Thời Gian Chạy DB Query (giây)", color="red")

# Thêm tiêu đề và chú thích
plt.title("So sánh giữa CPU và GPU về Thời Gian Chạy DB Query và Giá Thuê AWS")
